...
<?php
    ...
    $uname = (isset($_GET['uname']) && !empty($_GET['uname'])) ? $_GET['uname'] : null;
    $passwd = (isset($_GET['verif']) && !empty($_GET['verif'])) ? base64_decode($_POST['verif']) : null;
    ...
    try{
        ...
        if($uname && $passwd){
            $sql = "SELECT id FROM participants WHERE uname = :uname";
            $stmt =$db->prepare($sql);
            ...
            if(!empty($ret)){
                $sql = "SELECT passwd FROM participants WHERE uname = :uname";
                $stmt =$db->prepare($sql);
                ...
                if(!$ret){
                    ...
                    $msg = "Something went wrong. You will be redirected to our homepage. Please try again.";
                }

            } else {
                $msg = "Your account doesn't seem to exist :(";
            }
